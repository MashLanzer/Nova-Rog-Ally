...
# LO QUE Test-CabeOtroAviso NECESITA HOY (25/09). Desde la idea 50 pasa por Get-SueloPorAnimo,
# y desde la idea 34 esa mira primero la ventana larga de siete dias. Sin estas lineas el banco
# probaba con las variables vacias y el suelo salia 6 en vez de 4.
# Este fichero prueba el SUELO, no el animo -de eso van probar-animo-consecuencias y
# probar-animo-largo-, asi que aqui se deja el animo neutro y sin base larga a proposito.
$script:uiAnimo = 0.0
$script:animoLargo = 0.0
$script:animoBase = 0
foreach ($cteA in @('AnimoMalo', 'AnimoBueno', 'AnimoLargoMinDias')) {
    $mA = [regex]::Match($fuente, ('(?m)^\$' + $cteA + '\s*=\s*(.+)$'))
    if ($mA.Success) { Invoke-Expression ('$' + $cteA + ' = ' + $mA.Groups[1].Value.Trim()) }
    else { Write-Host ("  MAL  no encuentro la constante " + $cteA); $script:fallos++ }
}
# y un doble de la cuenta del dia: este banco le da los numeros a mano, no los lee del disco
