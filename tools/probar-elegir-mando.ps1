...
# Y no es que no haya preguntas -hubo unas cuarenta confirmaciones- ni que no haya mando: es
# una consola de mano, XInput lo ve en el puerto 0 y leerlo cuesta 0,197 ms (medido hoy).
# Seis de esas confirmaciones murieron por plazo, una de ellas "abre Hollow Knight en steam",
# que ya venia de un oido dudoso. El atajo existia desde el 13/09 y era invisible.
#
...
foreach ($v in @('EleccionMs', 'XINPUT_ARR', 'XINPUT_ABA', 'XINPUT_IZQ', 'XINPUT_DER',
                 'XINPUT_A', 'XINPUT_B')) { Invoke-Expression (TraerVar $v) }
...
# una vuelta del bucle con estos botones recien pulsados
function Vuelta([int]$pulsados = 0, [int]$avanzaMs = 0) {
...
# y la regla de verdad, la del 13/09, sigue en el codigo
